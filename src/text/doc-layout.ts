...
export interface LineBox {
  ...
  glyphStart?: number; glyphEnd?: number;
  decoStart?: number; decoEnd?: number;
  hlStart?: number; hlEnd?: number;
}

/** 原子块的覆盖层类型（图片/公式/表格/形状/音频/视频/内嵌网页/附件/电子签名/印章/文本框）。@public */
export type OverlayKind = 'image' | 'formula' | 'table' | 'shape' | 'audio' | 'video' | 'iframe' | 'attachment' | 'signature' | 'seal' | 'textbox';
 ...
 */
export interface InlineOverlayBox { block: number; offset: number; kind: 'image'; x: number; y: number; w: number; h: number }
...
interface El { ch: string; style: Style; underline: RGBA | null; strike: RGBA | null; highlight: RGBA | null; baselineShift: number; uStart: number; atomW?: number; atomH?: number }
...
function buildTextBlockGeom(
  blk: Block, rb: ResolvedBlock, orderedNum: number, version: number,
  shaper: Shaper, resolver: StyleResolver, opt: DocLayoutOpts,
  ...
  for (const run of blk.inlines) {
    ...
    if (isInlineAtom(run)) {
      ...
      els.push({ ch: run.text, style: rb.base, underline: null, strike: null, highlight: null, baselineShift: 0, uStart: uOff, atomW, atomH });
  ...
  for (let i = 0; i < els.length; i++) if (els[i].atomW !== undefined) shaped[i] = { ...shaped[i], advance: els[i].atomW! };
  ...
  for (let li = 0; li < lineRuns.length; li++) {
    ...
    const gStart = glyphs.length, dStart = decorations.length, hStart = highlights.length;
    // 行度量
    ...
    for (const ei of le) {
      ...
      if (aH !== undefined) { ascent = Math.max(ascent, aH); lineH = Math.max(lineH, aH + descent); }
    }
    void descent;
    // 行距：自然行高 × lineHeight 倍数；多余行距等分到上下，使文字垂直居中于行盒
    lineH *= rb.lineHeight;
    const lead = lineH - (ascent + descent); // 自然行高之外的多余行距（含 lineHeight 放大量）
    const top = y, bottom = y + lineH, baseline = y + ascent + Math.max(0, lead) / 2;
    ...
    let lineW = 0; for (const a of adv) lineW += a;
    ...
    if (align === 'center') startX = x0 + slack / 2;
      ...
      if (align === 'justify') {
        ...
        if (spaceIdx.length > 0) { const add = slack / spaceIdx.length; for (const k of spaceIdx) adv[k] += add; }
      } else {
        // distribute：n 元素有 n-1 个字间，slack 均分到每个间隙（加到除末元素外的每个 advance）
        if (le.length > 1) { const add = slack / (le.length - 1); for (let k = 0; k < le.length - 1; k++) adv[k] += add; }
      }
    }
    ...
    for (let v = 0; v < vorder.length; v++) {
      ...
      const left = penX, right = penX + cell;
      ...
      if (e.atomW !== undefined && e.atomH !== undefined) {
        inlineOverlays.push({ offset: e.uStart, kind: 'image', x: left, y: baseline - e.atomH, w: e.atomW, h: e.atomH });
        penX = right;
        continue;
      }
      ...
      if (e.underline) decorations.push({ x: left, y: elBaseline + Math.round(2 * opt.dpr), w: cell, h: decoH, color: e.underline });
      if (e.strike) decorations.push({ x: left, y: elBaseline - Math.round(shaper.fontMetrics(e.style).ascent * 0.32), w: cell, h: decoH, color: e.strike });
      penX = right;
    }
    if (le.length === 0) caretMap.set(0, startX);

    // 视觉行边界 = 排字 pen 的起止（startX..penX），与排字/视觉顺序无关，精确且不依赖 xs 排序。
    const minX = Math.min(startX, penX), maxX = Math.max(startX, penX);
    ...
    lines.push({ top, bottom, baseline, startOffset: le.length ? els[le[0]].uStart : 0, endOffset: endOff, offsets, xs, minX, maxX, rtl });
  ...
  return { version, orderedNum, linesH: y, lines, glyphs, decorations, highlights, inlineOverlays, glyphRange, decoRange, hlRange };
}
 ...
 * @public
 */
export function layoutDoc(doc: Doc, shaper: Shaper, resolver: StyleResolver, opt: DocLayoutOpts, cache?: BlockLayoutCache): DocLayout {
  ...
  for (let bi = 0; bi < doc.blocks.length; bi++) {
    ...
    if (isAtomBlock(blk.type)) {
      ...
      lines.push({
        block: bi, top: y, bottom: y + h, baseline: y + h, startOffset: 0, endOffset: 0,
        offsets: [0], xs: [contentLeft], minX: contentLeft, maxX: contentLeft, rtl: false,
        glyphStart: glyphs.length, glyphEnd: glyphs.length,
        decoStart: decorations.length, decoEnd: decorations.length,
        hlStart: highlights.length, hlEnd: highlights.length,
      });
    ...
    if (blk.type === 'toc') {
      ...
      for (const row of tocRows) {
        ...
        const top = y, bottom = y + lineH, baseline = y + m.ascent;
        ...
        for (const s of shapedRow) {
          if (!s.glyph.empty) glyphs.push({ info: s.glyph, penX: penX + s.glyph.bearingX, baselineY: baseline, color: rb.base.color });
          penX += s.advance;
        }
        ...
        const ln: LineBox = {
          block: bi, top, bottom, baseline, startOffset: 0, endOffset: 0,
          offsets: [0], xs: [contentLeft], minX: contentLeft, maxX: contentLeft, rtl: rowRtl,
          glyphStart: gStart, glyphEnd: glyphs.length,
          decoStart: decorations.length, decoEnd: decorations.length,
          hlStart: highlights.length, hlEnd: highlights.length,
        };
    ...
    const gBase = glyphs.length, dBase = decorations.length, hBase = highlights.length;
    for (const pg of g.glyphs) glyphs.push({ info: pg.info, penX: pg.penX, baselineY: pg.baselineY + blockTop, color: pg.color });
    ...
    for (const ov of g.inlineOverlays) inlineOverlays.push({ block: bi, offset: ov.offset, kind: ov.kind, x: ov.x, y: ov.y + blockTop, w: ov.w, h: ov.h });
    for (let li = 0; li < g.lines.length; li++) {
      const rl = g.lines[li];
      lines.push({
        block: bi,
        top: rl.top + blockTop, bottom: rl.bottom + blockTop, baseline: rl.baseline + blockTop,
        startOffset: rl.startOffset, endOffset: rl.endOffset,
        offsets: rl.offsets, xs: rl.xs, minX: rl.minX, maxX: rl.maxX, rtl: rl.rtl,
        glyphStart: gBase + g.glyphRange[li][0], glyphEnd: gBase + g.glyphRange[li][1],
        decoStart: dBase + g.decoRange[li][0], decoEnd: dBase + g.decoRange[li][1],
        hlStart: hBase + g.hlRange[li][0], hlEnd: hBase + g.hlRange[li][1],
      });
    }
    y += g.linesH;
  ...
  return { backgrounds, highlights, glyphs, decorations, overlays, inlineOverlays, lines, contentHeight: y - opt.padT, contentRight, dpr: opt.dpr };
}
...
export function visibleLineRange(
  lineTops: ArrayLike<number>, lineBottoms: ArrayLike<number>,
