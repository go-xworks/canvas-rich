...
const GLYPH: GlyphInfo = {
  u0: 0, v0: 0, u1: 0.05, v1: 0.05, page: 0, w: 8, h: 12, bearingX: 1, bearingY: 10, advance: ADV, empty: false,
};
...
class CountingShaper implements Shaper {
  ...
  fontMetrics(style: Style): FontMetrics { return this.inner.fontMetrics(style); }
  shapeChars(chars: StyledChar[]): ShapedChar[] { this.calls++; return this.inner.shapeChars(chars); }
}
...
const EPOCH = (s: Shaper = shaper): LayoutEpoch =>
  ({ width: OPT.width, padL: OPT.padL, padT: OPT.padT, scale: OPT.dpr, shaper: s, theme: 'light', atlasGen: 0 });
const P_OPTS: PaginateOpts = {
  pageX: 4, pageW: 300, pageH: 200, marginTop: 16, marginBottom: 16, gap: 12, padT: 28,
};
...
function applyRandomEdit(rd: RichDoc, rng: () => number): void {
  ...
  const randSel = () => { rd.setSel(randPos()); rd.setSel(randPos(), true); };
  const ops: (() => void)[] = [
    () => { rd.setSel(randPos()); rd.insertText(INSERT_POOL[ri(INSERT_POOL.length)]); },
    () => { rd.setSel(randPos()); rd.backspace(); },
    () => { rd.setSel(randPos()); rd.del(); },
    () => { rd.setSel(randPos()); rd.enter(); },
    () => { randSel(); rd.toggleMark('bold'); },
    () => { randSel(); rd.toggleMark('underline'); },
    () => { randSel(); rd.toggleMark('highlight'); },
    () => { randSel(); rd.toggleExclusiveMark('superscript', ['superscript', 'subscript']); },
    () => { randSel(); rd.setMark('color', { color: '#ff0000' }); },
    () => { randSel(); rd.clearMarks(); },
    () => { randSel(); rd.setBlockType('heading', { level: ([1, 2, 3] as const)[ri(3)] }); },
    () => { randSel(); rd.setBlockType('code_block'); },
    () => { randSel(); rd.setBlockType('ordered_item'); },
    () => { randSel(); rd.setBlockType('paragraph'); },
    () => { randSel(); rd.setAlign('justify'); },
    () => { randSel(); rd.setAlign('center'); },
    () => { randSel(); rd.setIndent(ri(40)); },
    () => { randSel(); rd.setLineHeight(1 + rng()); },
    () => { randSel(); rd.setSpaceBefore(ri(24)); },
    () => { randSel(); rd.setLetterSpacing(ri(4)); },
    () => { randSel(); rd.setDir(rng() < 0.5 ? 'rtl' : 'ltr'); },
    () => { randSel(); rd.indentList(); },
    () => { randSel(); rd.outdentList(); },
    () => rd.moveBlock(ri(rd.blockCount), ri(rd.blockCount + 1)),
    () => { randSel(); rd.backspace(); }, // 选区删除（跨块时走 deleteSel 合并路径）
    () => rd.toggleTaskChecked(ri(rd.blockCount)),
    () => { rd.setSel(randPos()); rd.insertTable(2, 2); },
    () => { rd.setSel(randPos()); rd.insertInlineImage('z.png', 20, 16); },
    ...
    () => { // 富文本粘贴：单块/多块/原子首块三分支（随机片段形态）
      ...
      const frags: Doc[] = [
        ...
        { blocks: [{ type: 'image', attrs: { src: 'p.png', width: 40, height: 30 }, inlines: [text('')] }, para([text('after')])] },
      ];
    ...
    () => { randSel(); rd.moveSelTo(randPos()); }, // 拖拽移动选中文本（无效落点自返 false）
    () => rd.undo(),
    () => rd.redo(),
  ];
  ops[ri(ops.length)]();
...
describe('增量布局定向用例', () => {
  ...
  it('② 连续代码块共享背景跨缓存命中边界正确合并', () => {
    ...
    const rd = new RichDoc({
      blocks: [
        block('code_block', [text('l1')]),
        block('code_block', [text('l2')]),
        para([text('after')]),
      ],
    });
    cache.beginPass(EPOCH());
  ...
  it('④ epoch 任一字段变化（width/scale/theme/shaper/atlasGen）→ 缓存整体失效全量重建', () => {
    ...
    let expected = warm;
    for (const over of [
      { atlasGen: 1 }, { theme: 'dark' }, { scale: 2 }, { width: OPT.width + 8 }, { shaper: new FakeShaper() as Shaper },
