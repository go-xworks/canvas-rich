...
describe('isSafeCssColor 白名单', () => {
  it('接受 #hex（3/4/6/8 位）/ rgb()/rgba() / 具名色', () => {
    for (const v of ['#f00', '#f00a', '#ff0000', '#ff000080', 'rgb(31, 36, 48)', 'rgba(1,2,3,.5)',
      'rgb(1 2 3)', 'rgb(1 2 3 / 50%)', 'red', 'transparent', 'currentcolor']) {
      expect(isSafeCssColor(v), v).toBe(true);
    }
  });
  it('拒绝 CSS 注入与畸形值', () => {
    for (const v of ['x;position:fixed', '#ff0000;inset:0', 'rgb(1,2,3);background:url(http://evil)',
      'url(http://evil/leak)', 'expression(alert(1))', 'var(--x)', 'javascript:alert(1)', '#ff00zz', '', 'rgb(', 'red green blue extra words here exceed']) {
...
describe('isSafeFontFamily 安全字符集', () => {
  it('接受常规族名（含引号串/逗号回退链/CJK 字体名）', () => {
    for (const v of ['Georgia', 'serif', 'monospace', '"Microsoft YaHei", sans-serif',
      "'PingFang SC', sans-serif", '微软雅黑', 'Noto Sans CJK SC', 'Segoe UI-Variable']) {
      expect(isSafeFontFamily(v), v).toBe(true);
    }
  });
  it('拒绝含 CSS 元字符（; : ( )）的注入值与空串', () => {
    for (const v of ['Arial;position:fixed;inset:0;background:url(http://evil/leak)',
      'x;color:red', 'a:b', 'fn(1)', 'url(http://e)', '']) {
