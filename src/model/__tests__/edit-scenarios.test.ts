...
describe('插入场景', () => {
  ...
  it('选区跨越原子块删除 → 整体删除', () => {
    const rd = rdOf([
      para([text('abc')]),
      { type: 'image', attrs: { src: 'x' }, inlines: [text('')] },
      para([text('xyz')]),
    ]);
  ...
  for (let seed = 1; seed <= 80; seed++) {
    it(`seed ${seed}`, () => {
      ...
      for (let step = 0; step < 60; step++) {
        ...
        if (r() < 0.4) {
          const b2 = Math.floor(r() * rd.blockCount);
          rd.setSel({ block: b2, offset: Math.floor(r() * (rd.blockLen(b2) + 1)) }, true);
        }
        const op = r();
        if (op < 0.2) {
          rd.insertText('ab');
          ...
          rd.toggleMark(MARKS[Math.floor(r() * MARKS.length)]);
          ops.push('mark');
        } else if (op < 0.68) {
          rd.setBlockType(BLOCKS[Math.floor(r() * BLOCKS.length)]);
          ops.push('blk');
        } else if (op < 0.73) {
          rd.insertImage('x');
          ops.push('img');
  ...
  for (let seed = 1; seed <= 30; seed++) {
    it(`seed ${seed}`, () => {
      ...
      for (let i = 0; i < N; i++) {
        const bi = Math.floor(r() * rd.blockCount);
        rd.setSel({ block: bi, offset: Math.floor(r() * (rd.blockLen(bi) + 1)) });
        if (r() < 0.6) rd.insertText('z');
        else rd.enter(); // 两者都必然快照
      }
