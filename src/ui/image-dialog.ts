...
export function createImageDialog(): ImageDialog {
  ...
  card.setAttribute('aria-label', '插入图片');
  card.className =
    'w-[min(460px,92vw)] bg-[var(--rte-overlay-bg)] border border-[var(--rte-overlay-border)] ' +
  ...
  body.className = 'px-4 py-2 flex flex-col gap-2.5';
  ...
  drop.type = 'button';
  drop.className =
    'w-full rounded-lg border-2 border-dashed border-[var(--rte-overlay-border)] bg-[var(--rte-canvas)] ' +
  ...
  drop.innerHTML =
    icon('image', 26) + '<span>拖拽图片到此，或<span class="text-[var(--rte-accent)]"> 点击选择文件</span></span>';

  const urlInput = document.createElement('input');
  urlInput.type = 'text';
  urlInput.placeholder = '或粘贴图片链接 https://…';
  urlInput.spellcheck = false;
  urlInput.setAttribute('aria-label', '图片链接');
  urlInput.className =
    'h-[34px] px-2.5 rounded-md border border-[var(--rte-overlay-border)] bg-[var(--rte-canvas)] ' +
    'text-[var(--rte-text)] text-[13px] outline-none appearance-none focus:border-[var(--rte-accent)]';

  const preview = document.createElement('div');
  preview.className =
    'hidden rounded-md border border-[var(--rte-overlay-border)] bg-[var(--rte-code-bg)] p-2 items-center justify-center';
  const previewImg = document.createElement('img');
  previewImg.alt = '预览';
  previewImg.className = 'max-h-[140px] max-w-full object-contain rounded';
  ...
  drop.ondragover = (e) => {
    e.preventDefault();
    drop.classList.add('border-[var(--rte-accent)]');
  };
  drop.ondragleave = () => drop.classList.remove('border-[var(--rte-accent)]');
  drop.ondrop = (e) => {
    e.preventDefault();
    drop.classList.remove('border-[var(--rte-accent)]');
