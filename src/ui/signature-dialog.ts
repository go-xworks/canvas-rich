...
export function createSignatureDialog(): SignatureDialog {
  ...
  card.setAttribute('role', 'dialog'); card.setAttribute('aria-modal', 'true'); card.setAttribute('aria-label', '手写签名');
  card.className = 'w-[min(480px,94vw)] bg-[var(--rte-overlay-bg)] border border-[var(--rte-overlay-border)] '
  ...
  const body = document.createElement('div'); body.className = 'px-4 py-2 flex flex-col gap-2';

  // 画板：CSS 尺寸固定、物理像素按 dpr 放大；透明底（toDataURL 保留透明，便于叠加文档）。
  const pad = document.createElement('canvas');
  pad.className = 'w-full rounded-md border border-[var(--rte-overlay-border)] bg-[var(--rte-canvas)] touch-none cursor-crosshair';
  ...
  clearBtn.className = 'px-3 py-1.5 mr-auto rounded-md border border-[var(--rte-overlay-border)] bg-transparent '
  ...
  okBtn.className = 'px-3.5 py-1.5 rounded-md border-0 bg-[var(--rte-accent)] text-white text-[13px] cursor-pointer '
  ...
  const reset = (): void => {
    ...
    c.lineWidth = LINE_W; c.lineCap = 'round'; c.lineJoin = 'round'; c.strokeStyle = INK;
    dirty = false;
    okBtn.disabled = true;
  };
  ...
  pad.addEventListener('pointerdown', (e) => {
    e.preventDefault();
    try { pad.setPointerCapture(e.pointerId); } catch { /* 合成事件/无效指针 */ }
    drawing = true;
    const { x, y } = localXY(e);
    const c = ctx();
    c.beginPath(); c.moveTo(x, y);
    // 单击点也留一个点（极短笔画），使「点一下」也算签名内容
    c.lineTo(x + 0.01, y + 0.01); c.stroke();
    dirty = true; okBtn.disabled = false;
  });
  ...
  clearBtn.onclick = (e) => { e.preventDefault(); reset(); };
  cancelBtn.onclick = (e) => { e.preventDefault(); close(null); };
  okBtn.onclick = (e) => { e.preventDefault(); if (dirty) close(pad.toDataURL('image/png')); };
  scrim.addEventListener('mousedown', (e) => { if (e.target === scrim) close(null); });
  scrim.addEventListener('keydown', (e) => { if (e.key === 'Escape') { e.preventDefault(); close(null); } });
