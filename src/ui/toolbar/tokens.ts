...
/** 工具栏宿主外壳类。 @internal */
export const HOST =
  'flex flex-col bg-[var(--rte-chrome-bg)] border-b border-[var(--rte-chrome-border)] ' + 'font-sans select-none';
// 页签栏：一行文字页签，右端常驻区。
/** 页签栏容器类。 @internal */
...
/** 单个页签按钮类。 @internal */
export const TAB =
  'relative px-3 h-full bg-transparent border-0 appearance-none cursor-pointer ' +
...
/** 功能组容器类。 @internal */
export const GROUP =
  'flex items-center gap-1 px-2 self-stretch border-r border-[var(--rte-chrome-border)] ' + 'last:border-r-0';
// 组内两行紧凑布局容器。
/** 组内两行布局容器类。 @internal */
...
/** 方形图标按钮类。 @internal */
export const BTN =
  'w-[28px] h-[28px] rounded-md bg-transparent border-0 appearance-none ' +
...
/** 下拉面板容器类。 @internal */
export const PANEL =
  'absolute left-0 top-[32px] z-50 p-2 bg-[var(--rte-overlay-bg)] ' +
  'border border-[var(--rte-overlay-border)] rounded-lg shadow-[var(--rte-shadow)] hidden';
/** 颜色色块按钮类。 @internal */
export const SWATCH =
  'w-[22px] h-[22px] rounded-[5px] border border-black/15 cursor-pointer p-0 ' +
...
export const SHAPE_DEFS: [string, string, string][] = [
  ['line', 'sh-line', '直线'],
  ['rect', 'sh-rect', '矩形'],
  ['rounded-rect', 'sh-rounded', '圆角矩形'],
  ['ellipse', 'sh-ellipse', '椭圆'],
  ['triangle', 'sh-triangle', '三角形'],
  ['diamond', 'sh-diamond', '菱形'],
  ['star', 'sh-star', '五角星'],
  ['arrow', 'sh-arrow', '箭头'],
  ['divider', 'sh-divider', '分隔线'],
];

// 下拉菜单项（行选择）通用样式。
/** 下拉菜单项通用类。 @internal */
export const MENU_ITEM =
  'w-full text-left px-2.5 h-[28px] rounded-md bg-transparent border-0 appearance-none ' +
...
export const HEX_INPUT =
  'mt-1.5 w-full h-[26px] px-2 rounded-md border border-[var(--rte-overlay-border)] ' +
...
export const BLOCK_DEFS: [string, string, string, string][] = [
  ...
  ['heading1', 'heading-1', '标题 1 ⌘⌥1', '标题 1'],
  ['heading2', 'heading-2', '标题 2 ⌘⌥2', '标题 2'],
  ['heading3', 'heading-3', '标题 3 ⌘⌥3', '标题 3'],
  ['heading4', 'heading-4', '标题 4 ⌘⌥4', '标题 4'],
  ['heading5', 'heading-5', '标题 5 ⌘⌥5', '标题 5'],
  ['heading6', 'heading-6', '标题 6 ⌘⌥6', '标题 6'],
  ['blockquote', 'quote', '引用 ⌘⌥Q', '引用'],
  ['code_block', 'square-code', '代码块', '代码块'],
];

/** 列表块快捷按钮定义：[value, 图标名, 标题]。 @internal */
export const LIST_DEFS: [string, string, string][] = [
  ['bullet_item', 'list', '项目符号 ⌘⌥8'],
  ['ordered_item', 'list-ordered', '编号列表 ⌘⌥9'],
...
export const ALIGN_DEFS: [string, string, string][] = [
  ['left', 'align-left', '左对齐 ⌘⇧L'],
  ['center', 'align-center', '居中 ⌘E'],
  ['right', 'align-right', '右对齐 ⌘⇧R'],
  ['justify', 'align-justify', '两端对齐'],
  ['distribute', 'align-distribute', '分散对齐'],
];
