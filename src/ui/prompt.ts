...
export function createPromptDialog(): PromptDialog {
  ...
  card.className =
    'w-[min(440px,92vw)] bg-[var(--rte-overlay-bg)] border border-[var(--rte-overlay-border)] ' +
  ...
  okBtn.className =
    'px-3.5 py-1.5 rounded-md border-0 bg-[var(--rte-accent)] text-white text-[13px] ' +
  ...
  return {
    ask(opts: PromptOptions): Promise<string | null> {
      ...
      return new Promise<string | null>((resolve) => {
        ...
        body.innerHTML = '';
        const f: HTMLInputElement | HTMLTextAreaElement = opts.multiline
        ...
        const base =
          'w-full px-2.5 py-2 rounded-md border border-[var(--rte-overlay-border)] ' +
