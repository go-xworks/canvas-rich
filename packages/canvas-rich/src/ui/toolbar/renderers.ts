...
import { BTN, BTN_TEXT, PANEL, SWATCH, MENU_ITEM, HEX_INPUT, SWATCHES, BLOCK_DEFS, setOn } from './tokens';
import type {
  ToolbarContext,
  ToolbarState,
  ToolbarItem,
...
export function swatchFill(ctx: ToolbarContext, onPick: (hex: string | null) => void) {
  return (panel: HTMLElement, close: () => void): void => {
    ...
    clearItem.className =
      'mt-1.5 w-full h-[26px] rounded-md border border-[var(--rte-overlay-border)] ' +
    ...
    hex.type = 'text';
    hex.className = HEX_INPUT;
    hex.placeholder = '#2563eb';
...
export function menuFill(
  ...
  return (panel: HTMLElement, close: () => void): void => {
    ...
    if (clearLabel) {
      ...
      clr.type = 'button';
      clr.className =
        MENU_ITEM + ' mt-0.5 border-t border-[var(--rte-overlay-border)] rounded-none text-[var(--rte-muted)]';
...
export function blockMenuFill(ctx: ToolbarContext, onPick: (value: string) => void) {
  return (panel: HTMLElement, close: () => void): void => {
    ...
    for (const [val, ic, , short] of BLOCK_DEFS) {
      const item = document.createElement('button');
      item.type = 'button';
      item.className = MENU_ITEM + ' gap-2';
...
export function tableGridFill(ctx: ToolbarContext, onPick: (rows: number, cols: number) => void) {
  return (panel: HTMLElement, close: () => void): void => {
    const COLS = 10,
      ROWS = 8;
...
export function tplFill(ctx: ToolbarContext) {
  return (panel: HTMLElement, close: () => void): void => {
    ...
    const rebuild = (): void => {
      ...
      save.type = 'button';
      save.className =
        MENU_ITEM + ' mt-0.5 border-t border-[var(--rte-overlay-border)] rounded-none gap-2 text-[var(--rte-accent)]';
      save.innerHTML = icon('save', 15) + '<span>设为模板…</span>';
      save.onclick = (e) => {
        e.preventDefault();
        close();
        ctx.exec('template.save');
      };
...
export function numInput(
  ...
  const box = document.createElement('div');
  box.className =
    'inline-flex items-center gap-1 h-[26px] px-1.5 rounded-md ' +
    'border border-[var(--rte-overlay-border)] focus-within:border-[var(--rte-accent)]';
  const tag = document.createElement('span');
  tag.className = 'text-[11px] text-[var(--rte-muted)] whitespace-nowrap';
  ...
  inp.className =
    'w-[34px] h-full bg-transparent text-[var(--rte-chrome-fg)] text-[12px] ' +
    'outline-none border-0 tabular-nums text-right';
  const unit = document.createElement('span');
  unit.className = 'text-[11px] text-[var(--rte-muted)]';
  unit.textContent = 'px';
  const commit = (): void => {
...
export function renderTextButton(item: TextButtonItem, ctx: ToolbarContext): MountedItem {
  const b = document.createElement('button');
  b.type = 'button';
  b.className = item.className ?? BTN_TEXT;
  b.title = item.title;
  b.setAttribute('aria-label', item.title);
  // 初始内容：dynamic 控件由 refresh 回填；静态控件按 iconName + text 装配。
  if (!item.dynamic) {
    b.innerHTML =
      (item.iconName ? ctx.icon(item.iconName) : '') + (item.text != null ? `<span>${item.text}</span>` : '');
  }
...
export function renderColorDropdown(item: ColorDropdownItem, ctx: ToolbarContext): MountedItem {
  const dd = makeDropdown(
    ...
    item.iconName,
    item.title,
    swatchFill(ctx, (hex) => ctx.exec(item.command, hex)),
  );
  return {
    el: dd.box,
...
export function renderGridDropdown(item: GridDropdownItem, ctx: ToolbarContext): MountedItem {
  const dd = makeDropdown(
    ...
    item.iconName,
    item.title,
    tableGridFill(ctx, (rows, cols) => ctx.exec(item.command, { rows, cols })),
    false,
  );
  return { el: dd.box };
}
