...
describe('谓词纯函数对照源 refresh 分支', () => {
  ...
  it('color/highlight 触发钮 isActive 随对应色是否为 null', () => {
    ...
    if (color.kind !== 'color-dropdown' || hl.kind !== 'color-dropdown')
      throw new Error('color/highlight 应为 color-dropdown');
...
describe('工具栏 item.command 与命令总线一致性（统一命令总线接线）', () => {
  ...
  const ITEMS_WITH_CMD = allItems.filter(
    (i): i is Extract<ToolbarItem, { command: string }> =>
      'command' in i && typeof (i as { command?: unknown }).command === 'string',
  );
  ...
  it('除 template-dropdown 外的控件均声明 command（无遗漏）', () => {
    for (const it of allItems) {
      ...
      expect(
        'command' in it && typeof (it as { command?: unknown }).command === 'string',
        `item "${it.id}" 缺 command`,
      ).toBe(true);
    }
  });
