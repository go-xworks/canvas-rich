...
export const TIP_DESC: Record<string, string> = {
  ...
  重做: '重做被撤销的操作',
  正文: '设为普通段落',
  标题: '设为 1–6 级标题，组织文档结构',
  引用: '设为引用块（缩进 + 斜体弱化）',
  代码块: '设为等宽代码块（连续多行共背景）',
  项目符号: '无序列表，Tab / Shift+Tab 调整层级',
  编号列表: '有序列表，自动连续编号',
  任务列表: '可勾选待办项（☐ / ☑，点击切换）',
  粗体: '把选中文字加粗',
  斜体: '把选中文字变斜体',
  ...
  上标: '把选中文字变上标（如 x²）',
  下标: '把选中文字变下标（如 H₂O）',
  文字颜色: '设置选中文字的颜色（含自定义 hex）',
  高亮颜色: '给选中文字加背景高亮（含自定义 hex）',
  清除格式: '清除选区内全部行内格式',
  字号: '设置选中文字的字号',
  字体族: '设置选中文字的字体',
  ...
  居中: '段落居中对齐',
  右对齐: '段落右对齐',
  两端对齐: '两端对齐：拉伸词间距填满整行（末行除外）',
  分散对齐: '分散对齐：字符均匀分布填满整行',
  ...
  插入表格: '可视网格选行列数后插入表格',
  插入形状: '插入形状（线 / 矩形 / 椭圆 / 箭头等 9 种）',
  插入音频: '插入音频播放器（媒体 URL）',
  插入视频: '插入视频播放器（媒体 URL，可缩放）',
  插入内嵌网页: '插入内嵌网页 iframe（URL，沙箱隔离，可缩放）',
  插入附件: '插入可下载文件卡片（URL + 文件名）',
  插入电子签名: '弹出画板手写签名，确定后作为图片插入（可缩放）',
  插入印章: '输入单位名生成红色圆形公章（可缩放）',
  插入文本框: '插入可编辑浮动文本框（直接键入，可缩放拖动）',
  链接: '给选中文字加超链接（⌘ / Ctrl + 点击打开）',
  插入目录: '插入目录，自动汇总全文标题，点击跳转',
  插入分隔线: '插入一条水平分隔线',
